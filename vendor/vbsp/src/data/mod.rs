    ...
    pub fn visible_clusters(&self, cluster: i16) -> BitVec<u8> {
        ...
        while cluster_index < self.cluster_count {
            if pvs_buffer[buffer_index] == 0 {
                ...
                for i in 0..8 {
                    let bit = 1 << i;
                    // ad2read patch: the last packed byte can carry bits
                    // past cluster_count; BitVec::set panics there
                    // (gm_construct's mirror room hit it).
                    if (packed & bit) == bit && cluster_index < self.cluster_count {
...
impl Packfile {
    ...
    /// ad2read patch: every file name in the pak, so callers can match
    /// case-insensitively (custom-packed textures keep the author's casing).
    pub fn names(&self) -> Vec<String> {
        self.zip.lock().unwrap().file_names().map(|n| n.to_owned()).collect()
    }
