...
impl ReadRelative for AnimationDescription {
    ...
    fn read(data: &[u8], header: Self::Header) -> Result<Self, ModelError> {
        ...
        // An animation kept in an external .ani block is not read. It gets no
        // tracks, which leaves its bones in the reference pose; this used to
        // be a todo!() that took the whole program down on such a model.
        while header.animation_block == 0 {
            let (animation, next_offset) = read_animation(data, offset, header.frame_count as usize)?;
