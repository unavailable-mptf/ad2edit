use crate::Vector;
use crate::{index_range, Transform3x4};
...
impl BodyPartHeader {
    pub fn model_indexes(&self) -> impl Iterator<Item = usize> {
        // The stride is the whole model record. It used to subtract
        // size_of::<FixedString<0>>(), which is not zero (ArrayString keeps a
        // length), so every model after a body part's first was read four
        // bytes early per step: empty names, no meshes, nonsense offsets.
