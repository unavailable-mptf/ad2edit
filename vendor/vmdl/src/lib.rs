...
impl Model {
    ...
    /// The meshes the game draws by default: bodygroup 0 everywhere.
    pub fn meshes(&self) -> impl Iterator<Item = Mesh> {
        self.meshes_with(&[])
    }

    /// The meshes the game draws for a choice of bodygroups: for every body
    /// part the model `chosen` names (its first when `chosen` is short or
    /// asks for one that is not there), at the highest level of detail. A
    /// body part is a bodygroup and only one of its models is ever shown at
    /// once; walking all of them as one flat list drew every variant on top
    /// of the others and paired meshes with the wrong strip data.
    pub fn meshes_with<'a>(&'a self, chosen: &'a [usize]) -> impl Iterator<Item = Mesh<'a>> {
        self.mdl
            .body_parts
            .iter()
            .zip(self.vtx.body_parts.iter())
            .enumerate()
            .filter_map(move |(part, (mdl_part, vtx_part))| {
                let wanted = chosen.get(part).copied().unwrap_or(0);
                let wanted = if wanted < mdl_part.models.len() && wanted < vtx_part.models.len() { wanted } else { 0 };
                Some((mdl_part.models.get(wanted)?, vtx_part.models.get(wanted)?))
            })
            .filter_map(|(mdl_model, vtx_model)| Some((mdl_model, vtx_model.lods.first()?)))
            .flat_map(move |(mdl_model, lod)| {
                mdl_model.meshes.iter().zip(lod.meshes.iter()).map(move |(mdl, vtx)| Mesh {
                    model_vertex_offset: mdl_model.vertex_offset as usize,
                    model_name: mdl_model.name.as_str(),
...
impl ReadRelative for String {
    ...
    fn read(data: &[u8], _header: Self::Header) -> Result<Self, ModelError> {
        // ad2read patch: names in real workshop MDLs are not always UTF-8
        // (models/rotarycannon/kw/20mmrac.mdl fails on its first byte). A
        // name is only ever displayed or used as a lookup key, so decode
        // lossily rather than refuse the whole model.
        let bytes: Vec<u8> = data.iter().copied().take_while(|byte| *byte != 0).collect();
