...
impl<const LEN: usize> TryFrom<[u8; LEN]> for FixedString<LEN> {
    ...
    fn try_from(name_buf: [u8; LEN]) -> Result<Self, Self::Error> {
        ...
        // ad2read patch: lossy, and truncated to fit the fixed buffer if the
        // replacement characters made it longer than LEN.
        let lossy = String::from_utf8_lossy(name);
        let mut fitted: &str = &lossy;
        while fitted.len() > LEN {
            let cut = fitted.char_indices().rev().nth(0).map(|(i, _)| i).unwrap_or(0);
            fitted = &fitted[..cut];
        }
        Ok(FixedString(ArrayString::from(fitted).map_err(|_| StringError::NotNullTerminated)?))
    }
}
...
impl Transform3x4 {
    /// The matrix as the file stores it: three rows of a rotation and, in
    /// the fourth column, a translation, in Source's own axes.
